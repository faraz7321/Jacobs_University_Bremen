...
// Function prototypes
void displayRules(); //Display rules of the game
objectType retrievePlay(char selection); //Set enum value based on selection
bool validSelection(char selection); //Check if selection is valid
void convertEnum(objectType object); //Convert enum to displayable text
objectType winningObject(objectType play1, objectType play2); //Decides the winning object
void gameResult(objectType play1, objectType play2, int& winner); //Declares winner of the current game
void displayResults(int gCount, int wCount1, int wCount2); //Display results of the game

int main() {
    //Step 1
    int gameCount, winCount1, winCount2, gamewinner; //variable to store games
    char response, selection1, selection2;
    objectType play1, play2;  //player1's and player2's selection
