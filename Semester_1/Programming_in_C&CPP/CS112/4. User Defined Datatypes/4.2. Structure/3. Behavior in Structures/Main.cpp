...
int main()
{
  ...
  // Considering you have either 32-bit or 64-bit system, the memory of float is 4 bytes, and memory of int is 4 bytes.
  ...
  // Hence, 8 bytes of memory is allocated for structure variable p1.
  ...
  // The members of structure variable (both attributes and functions) are accessed using a dot (.) operator for both assignment of and retrieval of value.
  // Suppose, you want to access the two functions of structure variable p1 and assign it or display the stored values. You can perform this task by using following code:

  cout << "\nStoring Information." << endl;
  p1.setAttributes(agePerson, salaryPerson);
